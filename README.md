...
- **チェックポイント/レジューム** - 実行途中の状態を保存し、中断後に再開可能（Ctrl+Cで安全に中断）
...
# チェックポイント付き実行（中断可能、再開可能）
task-composer run --checkpoint <FILE>
task-composer run -c <FILE>       # 短縮形

# 実行のみ（静的解析なし）
task-composer exec <FILE>
task-composer exec --checkpoint <FILE>  # チェックポイント付き
...
長時間実行されるDAGを途中で中断し、後から再開できます。

#### 基本的な使い方

```bash
# チェックポイント付きで実行
task-composer run --checkpoint workflow.json

# Ctrl+C で安全に中断（現在のタスク完了後に停止）
...
- チェックポイントファイル（`<file>.checkpoint.json`）に実行状態を保存
- 完了したタスクの結果、ループのイテレーション状態を保持
- 再開時は完了済みタスクをスキップし、未完了タスクから実行
- DAG定義が変更されている場合は警告を表示（ハッシュ検証）
- 実行完了後、チェックポイントファイルは自動削除
...
サブグラフ（DagExecutor）内でのループ実行もチェックポイントに保存されます。
中断後の再開時、サブグラフのループも正しい位置から再開します。
